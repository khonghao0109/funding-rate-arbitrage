...
// FetchKrakenDepth reads one futures book. levels is ignored: the venue has no
// limit parameter and always answers with everything.
func FetchKrakenDepth(ctx context.Context, source string, symbol Symbol, _ int) (DepthBook, error) {
...
func parseKrakenDepth(resp krakenDepthResponse, source string, symbol Symbol) (DepthBook, error) {
...
func parseKrakenDepthSide(venueSymbol string, raw [][]float64) ([]DepthLevel, error) {
	out := make([]DepthLevel, 0, len(raw))
	for _, entry := range raw {
		if len(entry) < 2 {
			return nil, fmt.Errorf("kraken depth %s: level %v has no price/size pair", venueSymbol, entry)
		}
		out = append(out, DepthLevel{PriceQuote: entry[0], QtyNative: entry[1]})
	}
	return out, nil
}
