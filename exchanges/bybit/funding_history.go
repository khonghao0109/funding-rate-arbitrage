...
// FetchFundingHistory walks the window backwards from its end.
...
func parseBybitFundingHistory(resp bybitFundingHistoryResponse, symbol exchanges.Symbol, window exchanges.FundingWindow) ([]exchanges.FundingHistoryRow, int64, error) {
	...
	out := make([]exchanges.FundingHistoryRow, 0, len(resp.Result.List))
	var oldestMs int64
