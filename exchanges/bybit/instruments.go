...
func fetchBybitInstruments(ctx context.Context, source, category, marketType string, symbols []exchanges.Symbol) ([]exchanges.Instrument, error) {
...
func parseBybitInstrument(resp bybitInstrumentsResponse, source, marketType string, s exchanges.Symbol) (exchanges.Instrument, bool, error) {
	...
	if resp.RetCode != 0 {
		return exchanges.Instrument{}, false, fmt.Errorf("%s %s: venue error retCode %d: %s", source, s.Venue, resp.RetCode, resp.RetMsg)
	}
	...
	inst := exchanges.Instrument{
		...
		Status:       exchanges.NormalizeStatus(e.Status, e.Status == "Trading"),
	...
	var err error
	if inst.TickSizeQuote, err = exchanges.ParseFloatField(source, e.Symbol, "tickSize", e.PriceFilter.TickSize); err != nil {
		return exchanges.Instrument{}, false, err
	}
	if inst.StepSizeCoin, err = exchanges.ParseFloatField(source, e.Symbol, stepField, stepValue); err != nil {
		return exchanges.Instrument{}, false, err
	}
	if inst.MinQtyCoin, err = exchanges.ParseFloatField(source, e.Symbol, "minOrderQty", e.LotSizeFilter.MinOrderQty); err != nil {
		return exchanges.Instrument{}, false, err
	}
	if inst.MaxQtyCoin, err = exchanges.ParseFloatField(source, e.Symbol, "maxOrderQty", e.LotSizeFilter.MaxOrderQty); err != nil {
		return exchanges.Instrument{}, false, err
	}
	if inst.MinNotionalQuote, err = exchanges.ParseFloatField(source, e.Symbol, notionalField, notionalValue); err != nil {
		return exchanges.Instrument{}, false, err
	}
	if e.LeverageFilter.MaxLeverage != "" {
		if inst.MaxLeverageX, err = exchanges.ParseFloatField(source, e.Symbol, "maxLeverage", e.LeverageFilter.MaxLeverage); err != nil {
			return exchanges.Instrument{}, false, err
		}
	}
