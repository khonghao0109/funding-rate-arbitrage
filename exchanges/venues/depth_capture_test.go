...
func TestCaptureDepthTestdata(t *testing.T) {
	if os.Getenv(exchangestest.CaptureEnv) != "1" {
		t.Skipf("set %s=1 to re-record order books from the live venues", exchangestest.CaptureEnv)
	}
	...
	for _, venue := range []struct {
		...
		},
		{
			...
			url: fmt.Sprintf("https://api.gateio.ws/api/v4/futures/%s/order_book?contract=%s&limit=%d",
				"usdt", captureVenueSymbol(t, "gate_futures"), levels),
		},
		{
