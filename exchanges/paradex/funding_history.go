...
const (
	...
	paradexFundingSampleEverySec = exchanges.SecPerHour
...
// FetchFundingHistory samples the funding index once an hour across the
...
func FetchFundingHistory(ctx context.Context, source string, symbol exchanges.Symbol, window exchanges.FundingWindow) ([]exchanges.FundingHistoryEntry, error) {
	gridMs := int64(paradexFundingSampleEverySec) * exchanges.MsPerSecond
	boundaryMs := (window.EndMs / gridMs) * gridMs

	var rows []exchanges.FundingHistoryRow
	for page := 0; page < exchanges.MaxFundingHistoryPages && boundaryMs > window.StartMs; page++ {
	...
	return exchanges.FinishFundingHistory(source, symbol, exchanges.FundingContinuous, rows)
}

func parseParadexFundingHistory(resp paradexFundingHistoryResponse, symbol exchanges.Symbol, window exchanges.FundingWindow) ([]exchanges.FundingHistoryRow, error) {
	out := make([]exchanges.FundingHistoryRow, 0, len(resp.Results))
	for _, entry := range resp.Results {
		...
		out = append(out, exchanges.FundingHistoryRow{
			...
			IntervalSec:  entry.FundingPeriodHours * exchanges.SecPerHour,
		})
	}
	return out, nil
}
