...
// FetchHyperliquidDepth reads one book. levels is ignored: the endpoint has no
// depth parameter and answers with 20 a side.
func FetchHyperliquidDepth(ctx context.Context, source string, symbol Symbol, _ int) (DepthBook, error) {
	payload := fmt.Sprintf(`{"type":"l2Book","coin":%q}`, symbol.Venue)
	var resp hyperliquidDepthResponse
	if err := postInstrumentJSON(ctx, "https://api.hyperliquid.xyz/info", payload, &resp); err != nil {
		return DepthBook{}, err
	}
	return parseHyperliquidDepth(resp, source, symbol)
}

func parseHyperliquidDepth(resp hyperliquidDepthResponse, source string, symbol Symbol) (DepthBook, error) {
	...
	if len(resp.Levels) != hyperliquidDepthSides {
		return DepthBook{}, fmt.Errorf("hyperliquid depth %s: %d sides in levels, want %d",
			symbol.Venue, len(resp.Levels), hyperliquidDepthSides)
	}

	book := DepthBook{Symbol: symbol.Standard, Source: source, VenueTimeMs: resp.TimeMs}
	for _, side := range []struct {
		into  *[]DepthLevel
		index int
	}{{&book.Bids, 0}, {&book.Asks, 1}} {
		for _, level := range resp.Levels[side.index] {
			parsed, err := parseDepthLevel(source, symbol.Venue, level.Px, level.Sz)
			if err != nil {
				return DepthBook{}, err
			}
