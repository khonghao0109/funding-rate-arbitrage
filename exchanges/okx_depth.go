...
// FetchOKXDepth reads one swap book.
func FetchOKXDepth(ctx context.Context, source string, symbol Symbol, levels int) (DepthBook, error) {
...
func parseOKXDepth(resp okxDepthResponse, source string, symbol Symbol) (DepthBook, error) {
	...
	if resp.Code != "0" {
		return DepthBook{}, fmt.Errorf("okx depth %s: code %s %s", symbol.Venue, resp.Code, resp.Msg)
	}
	if len(resp.Data) == 0 {
		return DepthBook{}, fmt.Errorf("okx depth %s: code 0 with no book in data", symbol.Venue)
	}
	...
	if entry.TS != "" {
		...
		if ms, err := parseInstrumentFloat(source, symbol.Venue, "ts", entry.TS); err == nil {
			book.VenueTimeMs = int64(ms)
		}
	}
	var err error
	if book.Bids, err = parseDepthSide(source, symbol.Venue, entry.Bids); err != nil {
		return DepthBook{}, err
	}
	if book.Asks, err = parseDepthSide(source, symbol.Venue, entry.Asks); err != nil {
		return DepthBook{}, err
	}
