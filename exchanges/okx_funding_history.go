...
// FetchOKXFundingHistory walks backwards from the end of the window.
func FetchOKXFundingHistory(ctx context.Context, source string, symbol Symbol, window FundingWindow) ([]FundingHistoryEntry, error) {
...
func parseOKXFundingHistory(resp okxFundingHistoryResponse, symbol Symbol, window FundingWindow) ([]fundingHistoryRow, int64, error) {
	...
	out := make([]fundingHistoryRow, 0, len(resp.Data))
	var oldestMs int64
