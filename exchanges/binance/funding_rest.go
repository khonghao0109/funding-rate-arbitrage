...
func runBinanceFunding(f exchanges.Feeds, source string, symbols []exchanges.Symbol) {
	meta := exchanges.NewFundingMetaCache()

	// Waited on, not fired and forgotten: this function returning is what tells
	// ConnectFutures that funding collection has stopped, and a poller
	// still inside a 30s HTTP call would make that claim false.
	var running sync.WaitGroup
	running.Add(1)
	go func() {
		defer running.Done()
		exchanges.PollFunding(f.Ctx, source, "fundingInfo", exchanges.FundingMetaEvery, func(ctx context.Context) error {
			return refreshBinanceFundingMeta(ctx, meta, symbols)
		})
	}()

	exchanges.PollFunding(f.Ctx, source, "premiumIndex", exchanges.FundingPollEvery, func(ctx context.Context) error {
		return pollBinancePremiumIndex(ctx, f, source, symbols, meta)
	})
	running.Wait()
}

func refreshBinanceFundingMeta(ctx context.Context, meta *exchanges.FundingMetaCache, symbols []exchanges.Symbol) error {
	var infos []binanceFundingInfo
	if err := exchanges.FetchJSON(ctx, "https://fapi.binance.com/fapi/v1/fundingInfo", &infos); err != nil {
		return err
	}
...
func binanceFundingMeta(infos []binanceFundingInfo, symbols []exchanges.Symbol) map[string]exchanges.FundingMetaEntry {
	...
	for _, s := range symbols {
		entry := exchanges.FundingMetaEntry{IntervalHours: binanceDefaultFundingIntervalHours}
...
func pollBinancePremiumIndex(ctx context.Context, f exchanges.Feeds, source string, symbols []exchanges.Symbol, meta *exchanges.FundingMetaCache) error {
	var errs []error
	for _, s := range symbols {
		var entry binancePremiumIndex
		err := exchanges.FetchJSON(ctx, "https://fapi.binance.com/fapi/v1/premiumIndex?symbol="+s.Venue, &entry)
...
func publishBinanceFunding(f exchanges.Feeds, source string, symbols []exchanges.Symbol, meta *exchanges.FundingMetaCache,
