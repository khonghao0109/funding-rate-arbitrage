...
// FetchDepth reads one futures book.
...
func parseGateDepth(resp gateDepthResponse, source string, symbol exchanges.Symbol) (exchanges.DepthBook, error) {
	book := exchanges.DepthBook{
		Symbol: symbol.Standard,
		Source: source,
		// Seconds with a fraction, to milliseconds. 0 stays 0.
		VenueTimeMs: int64(resp.CurrentSec * exchanges.MsPerSecond),
...
func parseGateDepthLevel(source, venueSymbol, price string, sizeContracts float64) (exchanges.DepthLevel, error) {
	priceQuote, err := strconv.ParseFloat(price, 64)
	if err != nil {
		return exchanges.DepthLevel{}, fmt.Errorf("gate depth %s: price %q does not parse", venueSymbol, price)
	}
	return exchanges.DepthLevel{PriceQuote: priceQuote, QtyNative: sizeContracts}, nil
}
