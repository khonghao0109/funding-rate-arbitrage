...
func ConnectFutures(source string, symbols []exchanges.Symbol, f exchanges.Feeds) {
	exchanges.RunStream(f, gateStream(source, symbols, f))
}

func gateStream(source string, symbols []exchanges.Symbol, f exchanges.Feeds) exchanges.StreamConfig {
		...
		Subscribe: func(conn *websocket.Conn) error {
			...
			for _, channel := range []string{"futures.book_ticker", "futures.tickers"} {
				err := conn.WriteJSON(GateSubscribeMessage{
					...
					Channel: channel,
					Event:   "subscribe",
					Payload: exchanges.VenueSymbols(symbols),
				})
				if err != nil {
					return err
				}
