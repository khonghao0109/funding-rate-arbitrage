...
func goldenConfigs(f exchanges.Feeds) map[string]exchanges.StreamConfig {
	return map[string]exchanges.StreamConfig{
		"okx_futures": okxStream("okx_futures", exchangestest.Symbols("okx_futures"), f),
	}
}
...
func TestGoldenFunding_HonoursTheContract(t *testing.T) {
	...
	cfg := goldenConfigs(r.Feeds)["okx_futures"]
	for _, frame := range exchangestest.ReadFrames(t, "okx_futures") {
		cfg.Handle(frame, recvAt)
	}
	exchangestest.CheckFundingContract(t, "okx_futures", r, recvAt, exchangestest.FundingExpectation{
...
func TestAcknowledgementProducesNoMarketData(t *testing.T) {
	r := exchangestest.NewRecorder(t)
	ack := []byte(`{"event":"subscribe","arg":{"channel":"books5","instId":"BTC-USDT-SWAP"},"connId":"b2400c07"}`)
	goldenConfigs(r.Feeds)["okx_futures"].Handle(ack, time.Now())
