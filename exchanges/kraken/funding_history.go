...
// FetchFundingHistory reads the whole published history in one request
...
func parseKrakenFundingHistory(resp krakenFundingHistoryResponse, symbol exchanges.Symbol, window exchanges.FundingWindow) ([]exchanges.FundingHistoryRow, error) {
	if resp.Result != "success" {
		return nil, fmt.Errorf("kraken funding history %s: result %q %s", symbol.Venue, resp.Result, resp.Error)
	}

	out := make([]exchanges.FundingHistoryRow, 0, len(resp.Rates))
