...
func FetchInstruments(ctx context.Context, source string, symbols []exchanges.Symbol) ([]exchanges.Instrument, error) {
	...
	if err := exchanges.FetchJSON(ctx, "https://futures.kraken.com/derivatives/api/v3/instruments", &resp); err != nil {
		return nil, err
	}
	return parseKrakenInstruments(resp, source, symbols)
}

func parseKrakenInstruments(resp krakenInstrumentsResponse, source string, symbols []exchanges.Symbol) ([]exchanges.Instrument, error) {
	...
	for _, e := range resp.Instruments {
		...
		inst := exchanges.Instrument{
			...
			Status:        exchanges.NormalizeStatus("untradeable", e.Tradeable),
			BaseAsset:     e.Base,
			QuoteAsset:    e.Quote,
