...
func pythSymbols() []Symbol {
	return []Symbol{{Standard: "BTCUSDT", Venue: pythBTCFeedID}}
}
...
func TestParsePythPrice_AppliesTheExponent(t *testing.T) {
	...
	for _, tc := range cases {
		got, err := ParsePythPrice(tc.raw, tc.expo)
		if err != nil {
			t.Errorf("ParsePythPrice(%q, %d): %v", tc.raw, tc.expo, err)
			continue
		}
		if math.Abs(got-tc.want) > math.Abs(tc.want)*1e-12 {
			t.Errorf("ParsePythPrice(%q, %d) = %g, want %g", tc.raw, tc.expo, got, tc.want)
		}
	}
}

func TestParsePythPrice_RejectsAPriceThatIsNotANumber(t *testing.T) {
	if _, err := ParsePythPrice("not-a-number", -8); err == nil {
