...
// FetchBybitFuturesDepth reads the linear (USDT perpetual) book.
func FetchBybitFuturesDepth(ctx context.Context, source string, symbol Symbol, levels int) (DepthBook, error) {
	return fetchBybitDepth(ctx, "linear", source, symbol, min(levels, bybitLinearDepthMaxLevels))
}

// FetchBybitSpotDepth reads the spot book.
func FetchBybitSpotDepth(ctx context.Context, source string, symbol Symbol, levels int) (DepthBook, error) {
	return fetchBybitDepth(ctx, "spot", source, symbol, min(levels, bybitSpotDepthMaxLevels))
}

func fetchBybitDepth(ctx context.Context, category, source string, symbol Symbol, levels int) (DepthBook, error) {
...
func parseBybitDepth(resp bybitDepthResponse, source string, symbol Symbol) (DepthBook, error) {
	...
	if bybitSaysSymbolNotListed(resp.RetCode, resp.RetMsg) {
		return DepthBook{}, fmt.Errorf("bybit depth %s: %w", symbol.Venue, errInstrumentNotListed)
	}
	if resp.RetCode != 0 {
		return DepthBook{}, fmt.Errorf("bybit depth %s: retCode %d %s", symbol.Venue, resp.RetCode, resp.RetMsg)
	}

	book := DepthBook{Symbol: symbol.Standard, Source: source, VenueTimeMs: resp.Result.TimestampMs}
	var err error
	if book.Bids, err = parseDepthSide(source, symbol.Venue, resp.Result.Bids); err != nil {
		return DepthBook{}, err
	}
	if book.Asks, err = parseDepthSide(source, symbol.Venue, resp.Result.Asks); err != nil {
		return DepthBook{}, err
	}
