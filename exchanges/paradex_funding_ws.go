...
func handleParadexFunding(source string, symbols []Symbol, f Feeds, raw []byte, recvAt time.Time) bool {
	var event paradexFundingEvent
	if !decode(raw, &event) || !strings.HasPrefix(event.Params.Channel, paradexFundingChannelPrefix) {
		return false
	}
	...
	standard := StandardOf(symbols, event.Params.Data.Market)
	if standard == "" {
