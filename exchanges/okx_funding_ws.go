...
func handleOKXFunding(source string, symbols []Symbol, f Feeds, raw []byte, recvAt time.Time) bool {
	var message okxFundingMessage
	if !decode(raw, &message) || message.Arg.Channel != "funding-rate" {
		return false
	}

	for _, entry := range message.Data {
		standard := StandardOf(symbols, entry.InstID)
		if standard == "" {
