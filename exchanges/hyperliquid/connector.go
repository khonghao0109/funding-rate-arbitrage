...
// ConnectFutures runs the market-data socket and, beside it, the
...
func ConnectFutures(source string, symbols []exchanges.Symbol, f exchanges.Feeds) {
	...
	go func() {
		defer running.Done()
		exchanges.PollFunding(f.Ctx, source, "predictedFundings", exchanges.FundingMetaEvery, func(ctx context.Context) error {
			return refreshHyperliquidFundingMeta(ctx, meta, symbols)
		})
	}()

	exchanges.RunStream(f, hyperliquidStream(source, symbols, f, meta))
	running.Wait()
}

func hyperliquidStream(source string, symbols []exchanges.Symbol, f exchanges.Feeds, meta *exchanges.FundingMetaCache) exchanges.StreamConfig {
...
func handleHyperliquidFrame(source string, symbols []exchanges.Symbol, meta *exchanges.FundingMetaCache, f exchanges.Feeds, raw []byte, recvAt time.Time) {
	...
	if exchanges.Decode(raw, &tradeMessage) && tradeMessage.Channel == "trades" && len(tradeMessage.Data) > 0 {
		...
		for _, trade := range trades {
			...
			symbol := exchanges.StandardOf(symbols, trade.Coin)
			if symbol == "" {
