...
func goldenConfigs(f exchanges.Feeds) map[string]exchanges.StreamConfig {
	return map[string]exchanges.StreamConfig{
		"hyperliquid_futures": hyperliquidStream("hyperliquid_futures", exchangestest.Symbols("hyperliquid_futures"), f, exchanges.NewFundingMetaCache()),
	}
}
...
func TestAcknowledgementProducesNoMarketData(t *testing.T) {
	r := exchangestest.NewRecorder(t)
	ack := []byte(`{"channel":"subscriptionResponse","data":{"method":"subscribe","subscription":{"type":"l2Book","coin":"BTC"}}}`)
	goldenConfigs(r.Feeds)["hyperliquid_futures"].Handle(ack, time.Now())
