...
func handleHyperliquidFunding(source string, symbols []Symbol, meta *fundingMetaCache,
	f Feeds, raw []byte, recvAt time.Time) bool {

	var message hyperliquidAssetCtxMessage
	if !decode(raw, &message) || message.Channel != "activeAssetCtx" {
		return false
	}

	standard := StandardOf(symbols, message.Data.Coin)
	if standard == "" {
	...
	data, err := normalizeHyperliquidFunding(hyperliquidFundingInput{
		...
		NextFundingAtMs: futureStampMs(
			entry.NextFundingAtMs+entry.IntervalHours*secPerHour*msPerSecond, recvAt),
	})
	if err != nil {
		return true
	}
...
func refreshHyperliquidFundingMeta(ctx context.Context, meta *fundingMetaCache, symbols []Symbol) error {
	// [[coin, [[venueName, {fundingRate, nextFundingTime, fundingIntervalHours}]]]]
	var raw [][]any
	if err := postInstrumentJSON(ctx, "https://api.hyperliquid.xyz/info", `{"type":"predictedFundings"}`, &raw); err != nil {
		return err
	}
...
// struct can describe.
func parseHyperliquidPredictedFundings(raw [][]any, symbols []Symbol) (map[string]fundingMetaEntry, error) {
	...
	for _, row := range raw {
		...
		for _, venueRow := range venues {
			...
			entry := fundingMetaEntry{IntervalHours: int64(hours)}
