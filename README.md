...
| **Đặt lệnh** | **Không đặt lệnh nào, kể cả trên testnet.** Bước 4.1 (2026-09-12, quyết định Q14) đã có hạ tầng REST **có ký** ở `internal/broker` — ký HMAC-SHA256, chỉnh lệch đồng hồ, ngân sách weight — nhưng CHỈ **testnet** (client từ chối mọi host khác, không có cờ mở mainnet trước 4.6) và chỉ **đọc**: `PlaceOrder`/`CancelOrder` thuộc 4.2 và chưa tồn tại. 4.1 đã **nghiệm thu 2026-09-13** (mã thoát 0, 4/4: đọc được số dư thật trên cả futures và spot testnet), và **Bước 4.2 nghiệm thu cùng ngày** — `internal/broker` có interface `Broker` và bản Binance đặt/huỷ được lệnh trên CẢ HAI sàn testnet (17/17, không lệnh nào khớp, vị thế và số dư đọc lại từ SÀN). **Bước 4.4a** (2026-09-13) đã có máy trạng thái mở hai chân trong `internal/execution` với đúng MỘT bất biến — sau khi mở thì hoặc cả hai chân mở, hoặc cả hai phẳng, không có trạng thái thứ ba — nhưng nó mới chỉ chạy với **broker giả** (8 ca có tên, 240 lần chạy ngẫu nhiên, 0 vi phạm). **Chưa một lệnh hai chân nào rời khỏi máy này** (sàn thật là 4.4b, vẫn sau phán quyết cổng 3.5) và **không thể chạm mainnet** (client từ chối mọi host khác, không có cờ mở trước 4.6). `cmd/scanner` không giữ credential và không import gói đó — kiểm bằng `go list -deps`, không bằng trí nhớ. Sổ giấy 4.3 định giá quyết định đã ghi bằng vốn ảo và **không nhìn thấy** khớp một phần, độ trễ, thanh lý hay sự cố API — đó là thứ 4.4/4.6 sinh ra để đo |
...
│   ├── broker/                # ⚠️ package DUY NHẤT giữ credential — 4.1/4.2, CHỈ testnet
